import React from 'react';
import { Alert, StyleSheet, Text, TouchableOpacity } from 'react-native';
import { Ionicons } from '@expo/vector-icons';
import {
  initPaymentSheet,
  initStripe,
  presentPaymentSheet,
} from '@stripe/stripe-react-native';
import { useAuth } from '../../contexts/AuthContext';
import { useRootLayout } from '../../contexts/RootLayoutContext';
import { authenticatedFetch } from '../../services/apiClient';
import { SmoothSpinner } from '../ui/LoadingState';
import { createIdempotencyKey } from '../../utils/idempotencyKey';
import { useSingleFlightAction } from '../../hooks/useSingleFlightAction';

type StripePaymentButtonProps = {
  disabled?: boolean;
  invoiceId: string;
  label?: string;
  onCompleted?: () => void;
};

type PaymentSession = {
  clientSecret: string;
  publishableKey: string;
};

export default function StripePaymentButton({
  disabled = false,
  invoiceId,
  label = 'Pay securely with Stripe',
  onCompleted,
}: StripePaymentButtonProps) {
  const { currentUser } = useAuth();
  const { brand, colors } = useRootLayout();

  const presentStripePayment = async () => {
    try {
      const session = await authenticatedFetch('/api/payments/create-intent', currentUser, {
        method: 'POST',
        retryCount: 0,
        body: {
          idempotencyKey: createIdempotencyKey('stripe-payment'),
          invoiceId,
          platform: 'native',
        },
      }) as PaymentSession;

      await initStripe({
        publishableKey: session.publishableKey,
        urlScheme: 'eduhub',
      });
      const initialization = await initPaymentSheet({
        merchantDisplayName: brand.name,
        paymentIntentClientSecret: session.clientSecret,
        returnURL: 'eduhub://stripe-redirect',
        style: brand.mode === 'light' ? 'alwaysLight' : 'alwaysDark',
      });
      if (initialization.error) {
        throw new Error(initialization.error.message);
      }

      const result = await presentPaymentSheet();
      if (result.error) {
        if (result.error.code === 'Canceled') return;
        throw new Error(result.error.message);
      }

      Alert.alert('Payment Submitted', 'Stripe is confirming your payment. Your ledger will update automatically.');
      onCompleted?.();
    } catch (error) {
      Alert.alert('Payment Unavailable', error instanceof Error ? error.message : 'The payment could not be started.');
    }
  };
  const { isPending: loading, run: pay } = useSingleFlightAction(presentStripePayment, {
    cooldownMs: 1200,
    disabled,
    haptic: 'medium',
  });

  return (
    <TouchableOpacity
      accessibilityLabel={label}
      accessibilityRole="button"
      disabled={disabled || loading}
      onPress={() => { void pay(); }}
      style={[
        styles.button,
        {
          backgroundColor: colors.violet,
          opacity: disabled || loading ? 0.55 : 1,
        },
      ]}
    >
      {loading ? (
        <SmoothSpinner color="#FFFFFF" size={24} style={undefined} trackColor="#CBD5E1" />
      ) : (
        <Ionicons name="card-outline" size={20} color="#FFFFFF" />
      )}
      <Text style={styles.label}>{loading ? 'Opening secure checkout...' : label}</Text>
    </TouchableOpacity>
  );
}

const styles = StyleSheet.create({
  button: {
    alignItems: 'center',
    borderRadius: 8,
    flexDirection: 'row',
    justifyContent: 'center',
    minHeight: 56,
    paddingHorizontal: 18,
  },
  label: {
    color: '#FFFFFF',
    fontSize: 15,
    fontWeight: '900',
    marginLeft: 8,
  },
});
